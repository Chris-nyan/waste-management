...
// 1. Add the 'Package' icon for waste types
import { Calendar, MapPin, Loader2, ServerCrash, Inbox, Clock, CheckCircle, XCircle, Package } from 'lucide-react';

import api from '@/lib/axios';
import AppLayout from '@/components/layout/AppLayout';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';
import { BookingDetailsModal } from '@/components/ui/BookingDetailModal';
...
const BookingCard = ({ booking, onViewDetails }) => {
    // Helper to format the waste types for display
    const formattedWasteTypes = booking.wasteTypes.map(w => w.charAt(0) + w.slice(1).toLowerCase()).join(', ');

    return (
        <div className="bg-white/50 backdrop-blur-sm border border-grey rounded-lg p-6 transition-all duration-300 hover:shadow-lg hover:border-emerald-300/50">
            ...
            <div className="space-y-3 text-sm text-gray-600 pt-4">
                ...
                {/* 2. Add the new line to display waste types */}
                <div className="flex items-center"><Package className="h-4 w-4 mr-3 text-primary shrink-0" /><span>{formattedWasteTypes}</span></div>
            </div>
...
const BookingCardSkeleton = () => (<div className="bg-white/50 backdrop-blur-sm border border-emerald-300/30 rounded-lg p-6 animate-pulse"><div className="flex justify-between items-center mb-4 pb-4 border-b border-emerald-200/50"><div className="space-y-2"><div className="h-3 bg-slate-200 rounded w-24"></div><div className="h-5 bg-slate-200 rounded w-40"></div></div><div className="h-6 bg-slate-200 rounded-full w-20"></div></div><div className="space-y-3 pt-4"><div className="h-4 bg-slate-200 rounded w-full"></div><div className="h-4 bg-slate-200 rounded w-5/6"></div><div className="h-4 bg-slate-200 rounded w-1/3"></div></div></div>);
